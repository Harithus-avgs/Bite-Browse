...
const RestaurantList = () => {
      ...
      if (country) {
        try {
          const response = await axios.get(
            `https://bite-browse.onrender.com/countries/${country}`
          );
    ...
    const fetchOptions = async () => {
      try {
        const cuisineResponse = await axios.get(
          `https://bite-browse.onrender.com/restaurants/cuisine-options?country_id=${countryCode}`
        );
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://bite-browse.onrender.com/restaurants?${queryParams.toString()}`)
      .then((response) => {
